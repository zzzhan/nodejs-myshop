...
module.exports = {
  add: function(user, txn, cb) {
	...
	sqlclient.queries([
	  ...
	  'UPDATE txn_journal set sum_amt=?,cust_id=?,remark=?,user_id=?,txn_detail=?,txn_date=? WHERE id=?',
	  'DELETE FROM txn_detail WHERE txn_id=?',
	  ...
	  [txn.txn_amt,txn.cust_id,txn.remark,user.id,txn.txn_details,txnDate],
	  [txn.txn_amt,txn.cust_id,txn.remark,user.id,txn.txn_details,txnDate,txn.id],
	  [txn.id],[arr],[]], 
		...
		switch(i) {
		  ...
		  case 5:
		    ...
		    for(var j=0;j<arg[0].length;j++) {
			  var prod = arg[0][j];
			  prod[prod.length] = !!txn.id?txn.id:results[2].insertId;
		    }
		    break;
		  case 6:
		    skip = !!txn.id;
