...
module.exports = {
  add: function(user, txn, cb) {
	...
	sqlclient.queries([
	  ...
	  'UPDATE customer set txn_count=txn_count+1 WHERE id=? AND name=?',
	  'INSERT INTO customer(name,user_id,txn_count,created,modified) VALUES(?,?,1,now(),now())',
	  'INSERT INTO txn_journal(sum_amt,cust_id,remark,user_id,txn_detail,created) VALUES(?,?,?,?,?,now())',
	...
	], [[txn.cust_id,txn.cust_name],[txn.cust_name,user.id],
	  [txn.sum_amt,txn.cust_id,txn.remark,user.id,txn.txn_details],[arr],[]], 
  ...
  list: function(data, cb) {
	...
	sqlclient.query('SELECT txn.id txn_id, sum_amt txn_amt,cust_id,cust.name cust_name,remark,txn.user_id,'+
		' DATE_FORMAT(txn.created,\'%Y-%m-%d\') txn_date,txn_detail'+
		' FROM txn_journal txn left join customer cust on txn.cust_id=cust.id'+
		' where txn.created between ? and ?'+(!!q?' and cust.name=?':''),[start, end, data.q], 
	  callback.handle(cb));
  }
};