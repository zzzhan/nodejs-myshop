...
module.exports = {
  ...
  list:function(cb, q) {
    sqlclient.query(
	  'SELECT id, name, type_id, unit_id, price, def_quantity, user_id, DATE_FORMAT(created,\'%Y-%m-%d\'), DATE_FORMAT(modified,\'%Y-%m-%d\') FROM product where status=0'+
	  (!!q?' AND name LIKE ? OR sname LIKE ?':'')+
	  ' ORDER BY txn_quantity DESC', [q+'%', q+'%'], callback.handle(cb));
  },
  add:function(user, data, cb) {
	...
	[[data.unit_name,user.id],[data.unit_name,user.id],[data.name,data.type_id,data.unit_id,data.price,
